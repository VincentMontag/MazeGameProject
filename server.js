...
console.log();
console.log("raceEnable -> can be either true or false. If it is false, the race is blocked and all currently active players are removed from the race.");
...
console.log("refreshTime -> with this setting, you can decide, after what time the labyrinth automatically refreshes. The time you choose will be in ms! Recommended refreshTime: 20000 ms");
//============================================================================================================
...
function setSetting(setting) {
	if (setting.s == 'raceEnable') {
		if (setting.v == 'true') setRaceEnable(true);
		else if (setting.v == 'false') setRaceEnable(false);
...
server.post("/markSleeping", (req, res) => {
	...
	if (session_id !== undefined && players1[session_id] !== undefined) {
		...
		if (racers[session_id] !== undefined) {
			if (RACE_WAIT_ON > 0) RACE_WAIT_ON++;
...
function startRaceOnConnection(id, socket) {
	if (!RACE_RUNNING) {
		if (id == WAIT_ON_CONNECTION) {
			WAIT_ON_CONNECTION = 0;
			for (key in racers) {
				racers[key].playerObject.wait = false;
				sendRaceWaitOn(racers[key].socket);	
			}
	...
	socket.onmessage = function incoming(event) {
		...
		if (action.dir == "") {
			...
			if (racers[action.id] !== undefined) {
				...
				startRaceOnConnection(action.id, socket);
			}
